...
const askKaruCommand: BotCommand = {
	...
	execute: async (interaction: ChatInputCommandInteraction) => {
		...
		if (!prompt) {
			return interaction.editReply(`${emojis.error} No question provided.`);
		}
...
Always assume the user wants high-signal help — no fluff.
			...
			await interaction.editReply({
				content: `${emojis.error} Failed to ask Kaeru. The system might be confused — try again in a moment.`,
			});
		}
	},
};
