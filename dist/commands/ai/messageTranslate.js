...
const messageTranslate = {
    ...
    execute: async (interaction) => {
        ...
        if (!message || typeof message.content !== "string" || message.content.trim() === "") {
            return sendErrorMessage(interaction, "This message seems to hold no content—nothing to translate so... this means nothing to translate. \n-# Message shouldn't be inside an embed or container telling it in case c:", "info");
        }
        try {
...
`.trim();
            ...
            const sectionOriginal = new TextDisplayBuilder().setContent(`### ${getEmoji("globe")} Original Message\n${formattedCleaned}`);
            const separator = new SeparatorBuilder()
                .setSpacing(SeparatorSpacingSize.Large)
                .setDivider(true);
            const sectionTranslated = new TextDisplayBuilder().setContent(`### ${getEmoji("swap")} Translated\n${formattedTranslated}`);
            ...
            return sendErrorMessage(interaction, "Failed to ask Karu. The system might be confused — try again in a moment.", "error");
        }
    },
};
